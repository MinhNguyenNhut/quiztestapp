...
export default function MultipleChoiceAnswers({ control, watch, setValue, index }: Props) {
  ...
  const handleCorrectToggle = useCallback(
    (optionIndex: number) => {
      const current = watchOptions?.[optionIndex]?.isCorrect || false;
      setValue(`questions.${index}.options.${optionIndex}.isCorrect`, !current, {
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  onChange={() => handleCorrectToggle(optIdx)}
                  size="small"
                />
              }
              label=""
